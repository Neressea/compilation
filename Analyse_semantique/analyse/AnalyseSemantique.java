	...
	private void createTDSFunc(CommonTree node){
		...
		if(node.getChildCount() == 4){
			...
			for (int i = 0; i < node.getChild(1).getChildCount(); i++) {
				...
				fv = new FieldVariable(p.getChild(0).getText(), - top().getCurrentSize() - 4, computeSizeType(p.getChild(1).getText()), p.getChild(1).getText());
				newTDS.add(fv);
			}
		...
		//Si on a que les paramï¿½tres qui sont prï¿½cisï¿½s
			...
			FieldVariable fv;
			for (int i = 0; i < node.getChild(1).getChildCount(); i++) {
				fv = new FieldVariable(node.getChild(1).getChild(i).getChild(0).getText(), - top().getCurrentSize() - 4, computeSizeType(node.getChild(1).getChild(i).getChild(1).getText()), node.getChild(1).getChild(i).getChild(1).getText());
				newTDS.add(fv);
			}
		}
	}
